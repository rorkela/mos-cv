...
void carrier_continuity(double *V, double *Vprev, double *nprev, double *pprev, double *n, double *p, int N) {
  ...
  for (int i = 0; i < N; i++)
    Vnorm[i] = V[i] / (kB * mos.T / q);
  for (int i = 0; i < N; i++)
    Vprevnorm[i] = Vprev[i] / (kB * mos.T / q);
  int maxiter = 20;
  int iter = 0;
...
void computeJacobi_n(double *Jac, double u, double *V, double *p, int N) {
  ...
  for (int i = 0; i < N; i++) {
    ...
    if (IN_OX(i) || i == 0) { // inside oxide conc is unchanging. Thus jacobian is such. Residual is zero respectively
      ...
      // DRi/Dni
      Jac[3*i+1]=mos.C_Rr*p[i]/2 + C*B(-V[i-1]+V[i]) + C+1/dt; //limiting case where the thing becomes 1 for constant
      potential
      // DRi/Dni+1
      Jac[3*i+2]=0;*/
      Jac[3 * i] = 0;
...
void computeJacobi_p(double *Jac, double u, double *V, double *n, int N) {
  ...
  for (int i = 0; i < N; i++) {
    ...
    if (IN_OX(i) || i == 0) { // inside oxide conc is unchanging. Thus jacobian is such. Residual is zero respectively
      ...
      /*Jac[3*i]=C*B(-V[i-1]+V[i]);
      // DRi/Dni
      Jac[3*i+1]=mos.C_Rr*n[i]/2 - C*B(V[i-1]-V[i]) - C+1/dt; //limiting case where the thing becomes 1 for constant
      potential
      // DRi/Dni+1
      Jac[3*i+2]=0;
      */
...
void residual_n(double *res, double *n, double *p, double *nprev, double *pprev, double *V, double *Vprev, double u,
                int N) {
  ...
  for (int i = 0; i < N; i++) {
    ...
    if (IN_OX(i) || i == 0) {
      ...
      res[i] = (n[i] - nprev[i]) / (sim.dt) - ((J[i] - J[i - 1] + Jprev[i] - Jprev[i - 1]) / (2 * q * mos.dx)) -
               (mos.Gr - mos.C_Rr * (n[i] * p[i] / 2 + nprev[i] * pprev[i] / 2 - mos.ni * mos.ni));
    }
  }
  free(J);
  free(Jprev);
...
void residual_p(double *res, double *n, double *p, double *nprev, double *pprev, double *V, double *Vprev, double u,
                int N) {
  // function to return the values of residual
  // used crank nicolson scheme for this (can be changed if not working)
  // we need current density for this
  ...
  for (int i = 0; i < N; i++) {
    ...
    if (IN_OX(i) || i == 0) {
      ...
      // sign of electron is handled
      // WARNING: the scheme changes for the function definition of compute_J
      res[i] = (p[i] - pprev[i]) / (sim.dt) + ((J[i] - J[i - 1] + Jprev[i] - Jprev[i - 1]) / (2 * q * mos.dx)) -
               (mos.Gr - mos.C_Rr * (n[i] * p[i] / 2 + nprev[i] * pprev[i] / 2 - mos.ni * mos.ni));
    }
  }
  free(J);
