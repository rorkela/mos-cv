#include "carrier.h"
#include "../main.h"
static inline double B(double z) {
    if (z > 35.0) {
        // For large positive z, B(z) -> 0
        return 0.0;
    } else if (z < -35.0) {
        // For large negative z, B(z) -> -z
        return -z;
    } else if (fabs(z) < 1e-9) {
        // For very small z, B(z) -> 1
        return 1.0;
    } else {
        // Use the precise formula only in the stable range
        return z / expm1(z);
    }
}
...
void computeJacobi_n(double *Jac,double u,double *V,double *p, int N){
  ...
  for(int i=0;i<N;i++){
    ...
    if(IN_OX(i)||i==0){ //inside oxide conc is unchanging. Thus jacobian is such. Residual is zero respectively
      ...
      Jac[3*i]=0;
      // DRi/Dni
      Jac[3*i+1]=mos.C_Rr*p[i]/2 + C*(V[i]-V[i+1])/((expm1(V[i]-V[i+1])))+1/dt;
      // DRi/Dni+1
      Jac[3*i+2]=-C*(-V[i]+V[i+1])/((expm1(-V[i]+V[i+1]))) ;
    }
    else if(i==N-1){ //At endpoint(Ohmic boundary condition)
      Jac[3*i]=-C*(V[i-1]-V[i])/((expm1(V[i-1]-V[i])));
      // DRi/Dni
      Jac[3*i+1]=mos.C_Rr*p[i]/2 + C*(-V[i-1]+V[i])/((expm1(-V[i-1]+V[i]))) + C+1/dt; //limiting case where the thing becomes 1 for constant potential
      // DRi/Dni+1
      Jac[3*i+2]=0;
      ...
      Jac[3*i]=-C*(V[i-1]-V[i])/((expm1(V[i-1]-V[i])));
      // DRi/Dni
      Jac[3*i+1]=mos.C_Rr*p[i]/2 + C*(-V[i-1]+V[i])/((expm1(-V[i-1]+V[i]))) + C*(V[i]-V[i+1])/((expm1(V[i]-V[i+1])))+1/dt;
      // DRi/Dni+1
      Jac[3*i+2]=-C*(-V[i]+V[i+1])/((expm1(-V[i]+V[i+1]))) ;
    }
...
void computeJacobi_p(double *Jac,double u,double *V,double *n, int N){
  ...
  for(int i=0;i<N;i++){
    ...
    if(IN_OX(i)||i==0){ //inside oxide conc is unchanging. Thus jacobian is such. Residual is zero respectively
      ...
      Jac[3*i]=0;
      // DRi/Dni
      Jac[3*i+1]=mos.C_Rr*n[i]/2 - C*(-V[i]+V[i+1])/((expm1(-V[i]+V[i+1])))+1/dt;
      // DRi/Dni+1
      Jac[3*i+2]=C*(V[i]-V[i+1])/((expm1(V[i]-V[i+1]))) ;
    }
    else if(i==N-1){ //At endpoint(Ohmic boundary condition)
      Jac[3*i]=C*(-V[i-1]+V[i])/((expm1(-V[i-1]+V[i])));
      // DRi/Dni
      Jac[3*i+1]=mos.C_Rr*n[i]/2 - C*(V[i-1]-V[i])/((expm1(V[i-1]-V[i]))) - C+1/dt; //limiting case where the thing becomes 1 for constant potential
      // DRi/Dni+1
      Jac[3*i+2]=0;
      ...
      Jac[3*i]=C*(-V[i-1]+V[i])/((expm1(-V[i-1]+V[i])));
      // DRi/Dni
      Jac[3*i+1]=mos.C_Rr*n[i]/2 - C*(V[i-1]-V[i])/((expm1(V[i-1]-V[i]))) - C*(-V[i]+V[i+1])/((expm1(-V[i]+V[i+1])))+1/dt;
      // DRi/Dni+1
      Jac[3*i+2]=C*(V[i]-V[i+1])/((expm1(V[i]-V[i+1]))) ;
    }
